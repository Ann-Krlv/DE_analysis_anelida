...
countData <- read.table("./Pdum_raw_counts_2_genes.tail.tsv", header=TRUE, sep="\t", row.name=1)
countData[1:12] <- lapply(countData[1:12], as.integer)

# design table
design <- data.frame(Point = colnames(countData),
                     TPA = rep(c(0, 4, 12, 24, 48, 96), each=2),
                     Replicates = rep(c(1:6), each = 2)
                     )
...
pcaData <- plotPCA(vsd, intgroup="TPA", returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
ggplot(pcaData, aes(PC1, PC2, color=TPA)) +
  geom_point(size=3) +
